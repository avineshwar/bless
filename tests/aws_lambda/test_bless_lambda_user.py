...
def test_basic_local_missing_kmsauth_request():
    output = lambda_handler_user(
        ...
        ca_private_key_password=RSA_CA_PRIVATE_KEY_PASSWORD,
        entropy_check=False,
        config_file=os.path.join(os.path.dirname(__file__), "bless-test-kmsauth.cfg"),
    )
    ...
    with pytest.raises(IOError):
        lambda_handler_user(
            ...
            config_file=os.path.join(
                os.path.dirname(__file__), "bless-test-broken.cfg"
            ),
        )
...
def test_local_request_with_test_user():
    output = lambda_handler_user(
        ...
        entropy_check=False,
        config_file=os.path.join(
            os.path.dirname(__file__), "bless-test-with-test-user.cfg"
        ),
    )
    assert output["certificate"].startswith("ssh-rsa-cert-v01@openssh.com ")
...
def test_local_request_with_custom_certificate_extensions():
    output = lambda_handler_user(
        ...
        entropy_check=False,
        config_file=os.path.join(
            os.path.dirname(__file__), "bless-test-with-certificate-extensions.cfg"
        ),
    )
    assert output["certificate"].startswith("ssh-rsa-cert-v01@openssh.com ")
...
def test_invalid_request_with_unallowed_remote():
    output = lambda_handler_user(
        ...
        entropy_check=False,
        config_file=os.path.join(
            os.path.dirname(__file__), "bless-test-kmsauth-different-remote.cfg"
        ),
    )
    assert output["errorType"] == "KMSAuthValidationError"
...
def test_valid_request_with_allowed_remote_and_allowed_iam_group(mocker):
    ...
    clientmock.list_groups_for_user.return_value = {
        "Groups": [{"GroupName": "ssh-alloweduser"}]
    }
    ...
    output = lambda_handler_user(
        ...
        entropy_check=False,
        config_file=os.path.join(
            os.path.dirname(__file__), "bless-test-kmsauth-iam-group-validation.cfg"
        ),
    )
    assert output["certificate"].startswith("ssh-rsa-cert-v01@openssh.com ")
...
def test_invalid_request_with_allowed_remote_and_not_allowed_iam_group(mocker):
    ...
    clientmock = mocker.MagicMock()
    clientmock.list_groups_for_user.return_value = {
        "Groups": [{"GroupName": "ssh-notalloweduser"}]
    }
    botomock = mocker.patch("boto3.client")
    botomock.return_value = clientmock
    output = lambda_handler_user(
